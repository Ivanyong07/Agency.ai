...
const Navbar = ({theme, setTheme}) => {
  ...
  return (
    ...
    sm:px-12 lg:px-24 xl:px-40 py-4 sticky top-0 z-20 bg-transparent 
    backdrop-blur-xl font-medium bg-white/30 dark:bg-black/30 dark:text-white'>

        <img src={theme === 'dark' ? assets.logo_dark : assets.logo} 
        className='w-32 sm:w-40' alt="" />

      <div className={`text-gray-700 dark:text-white sm:text-sm ${!sideBarOpen ?'max-sm:w-0 overflow-hidden' : `max-sm:w-60 
      max-sm:p-10`} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen 
      max-sm:h-full max-sm:flex-col max-sm:bg-primary max-sm:text-white text-gray-700 dark:text-white
      max-sm:pt-20 flex sm:items-center gap-5 transition-all`}>
        ...
        <div className='flex items-center gap-2 sm:gap-4'>
          ...
          <a href="#conect-us" className='text-sm max-sm:hidden 
